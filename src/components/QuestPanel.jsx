...
import { Target, Clock, CheckCircle, Sparkles, ChevronDown, ChevronUp } from 'lucide-react';

const QuestPanel = () => {
  const [quests, setQuests] = useState({ daily: [], weekly: [], recentCompleted: [] });
  ...
  const allActive = [...quests.daily, ...quests.weekly].filter(q => q.status === 'active');
          ...
          >
            <div className="px-4 pb-4 space-y-3">
              ...
              {allActive.length === 0 && completed.length === 0 && (
                <p className="text-sm text-muted-foreground text-center py-4">No quests available yet</p>
              )}
            </div>
          </motion.div>
        )}
...
const QuestItem = ({ quest, isCompleted = false }) => {
  const progress = quest.targetCount > 0 ? (quest.currentCount / quest.targetCount) * 100 : 0;

  return (
    <div className={`p-3 rounded-xl border mb-2 transition-colors ${
      isCompleted 
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between">
            <p className={`text-sm font-medium ${isCompleted ? 'text-green-400 line-through' : 'text-foreground'}`}>
              {quest.title}
            </p>
            <span className="text-xs font-bold text-primary ml-2 whitespace-nowrap">+{quest.xpReward} XP</span>
          </div>
