      ...
      <div className="min-h-screen bg-[#0f1923] flex flex-col items-center justify-center relative overflow-hidden">
        ...
        >
          ...
          <div
            ...
            style={{
              backgroundImage: `url(${profile?.avatar ? (profile.avatar.startsWith("data:") ? profile.avatar : profile.avatar.startsWith("http") ? profile.avatar : `${import.meta.env.VITE_API_URL || ""}${profile.avatar}`) : user?.avatar || "https://api.dicebear.com/7.x/avataaars/svg?seed=" + (user?.name || "Agent")})`,
            }}
          />
          ...
          {MODES.map((mode) => (
            ...
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent flex items-end justify-center pb-2">
                <span className="text-[10px] uppercase font-bold tracking-widest truncate max-w-full px-1">
                  {mode.name.split(" ")[0]}
                </span>
              </div>
            </motion.div>
