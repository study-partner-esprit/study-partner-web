...
const StudySessionSetup = () => {
  ...
  const handleJoinByCode = async () => {
    ...
    try {
      ...
      setJoinCodeError(err.response?.data?.error || "Invalid code or session not found");
    } finally {
    ...
    if (selectedType === "solo") {
      ...
      if (sid && selectedFriends.size > 0) {
        for (const friendId of selectedFriends) {
          try { await inviteToSession(sid, friendId); } catch { /* best-effort */ }
        }
      }
      navigate("/team-lobby");
    }
      ...
      <div className="relative z-20 h-20 px-8 flex items-center border-b border-[#ffffff10] bg-[#0f1923]/80 backdrop-blur-md">
        ...
        <div className="ml-auto flex items-center gap-4 text-sm font-bold tracking-widest text-gray-500">
          ...
          <span className={step === "course" ? "text-[#ff4655]" : ""}>
            COURSE
          </span>          {selectedType === "team" && (
            <>
              <ChevronRight size={14} />
              <span className={step === "friends" ? "text-[#ff4655]" : ""}>FRIENDS</span>
            </>
          )}          <ChevronRight size={14} />
              ...
              <div className="mt-8 max-w-2xl mx-auto">
                <div className="relative">
                  ...
                  <div className="relative flex justify-center">
                    <span className="px-3 bg-[#0f1923] text-xs text-gray-600 uppercase tracking-widest">or</span>
                  </div>
                </div>
                ...
                {!showJoinCode ? (
                  ...
                  >
                    <LogIn size={18} className="text-gray-600 group-hover:text-[#a78bfa] transition-colors" />
                  ...
                  >
                    ...
                    <div className="flex gap-3">
                      ...
                      >
                        {joinCodeLoading ? <Loader2 size={16} className="animate-spin" /> : <LogIn size={16} />}
                        JOIN
                      </button>
                      <button
                        onClick={() => { setShowJoinCode(false); setJoinCode(""); setJoinCodeError(""); }}
                    ...
                    {joinCodeError && (
                      <p className="mt-2 text-center text-xs text-red-400">{joinCodeError}</p>
                    )}
                  </motion.div>
                )}
              </div>
            ...
            >
              ...
              {coursesLoading ? (
                ...
                <div className="text-center py-20 text-gray-500">
                  ...
                  <p className="text-sm">Please upload a course first from the Subjects section to start studying</p>
                </div>
              ) : (
                  ...
                  {usableCourses.map((course, idx) => (
                    <motion.button
                      ...
                      className={`text-left p-5 rounded-xl border transition-all ${
                        (selectedCourse?._id || selectedCourse?.id) === (course._id || course.id)
                          ? "border-[#ff4655] bg-[#ff4655]/10"
                          : "border-[#ffffff10] bg-[#1a2633]/50 hover:border-[#ffffff30]"
                      }`}
                    >
                      <div className="flex items-start justify-between mb-1">
                        <h3 className="font-bold text-lg truncate flex-1">{course.title}</h3>
                      ...
                      <div className="flex items-center gap-4 text-xs text-gray-500">
                        <span className="flex items-center gap-1">
                          <BookOpen size={12} />
                          {course.topics?.length || course.topicsCount || 0} topics
                        </span>
                        <span className="flex items-center gap-1">
                          <Target size={12} />
          ...
          {step === "friends" && selectedType === "team" && (
            ...
            >
              ...
              <p className="text-gray-400 text-center mb-6 text-sm">
                Select up to 3 friends to invite — or skip and invite from the lobby
              </p>
              ...
              <div className="space-y-2 max-h-[45vh] overflow-y-auto pr-1 custom-scrollbar mb-6">
                {filteredFriends.length === 0 ? (
                  <div className="text-center py-10 text-gray-600">
                    ...
                    <p className="text-sm">{friends.length === 0 ? "No friends yet" : "No matches"}</p>
                  </div>
                ) : (
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="relative shrink-0">
                          ...
                          <div className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-[#0f1923] ${
                            friend.onlineStatus === "online" || friend.onlineStatus === "studying"
                              ? "bg-green-500"
                              : "bg-gray-600"
                          }`} />
                        </div>
                        ...
                        <div className="flex-1 text-left">
                          ...
                          <p className="text-[11px] text-gray-500 capitalize">
                            {friend.onlineStatus === "studying" ? "Currently studying" : friend.onlineStatus || "offline"}
                            {" • "}Lvl {friend.level || 1}
                          </p>
                        </div>
                        {/* Checkmark */}
              ...
              <div className="flex items-center gap-4">
                ...
                >
                  {selectedFriends.size > 0 ? "Continue →" : "Skip & Continue →"}
                </button>
              </div>
            </motion.div>
          )}
                ...
                {selectedType === "team" && selectedFriends.size > 0 && (
                  <div className="flex items-center justify-center gap-2 mb-4 flex-wrap">
                    {[...selectedFriends].map((fid) => {
                      const f = friends.find((fr) => (fr.userId || fr._id) === fid);
                      return f ? (
                        <span key={fid} className="flex items-center gap-1.5 text-xs bg-[#0fb8ce]/10 border border-[#0fb8ce]/30 text-[#0fb8ce] px-2.5 py-1 rounded-full font-medium">
                          <Users size={11} />
                          {f.name}
                        </span>
                      ) : null;
