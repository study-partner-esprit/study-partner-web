...
import { Brain, CheckCircle, RotateCcw, Clock, TrendingUp, Loader2, ChevronRight, Sparkles } from 'lucide-react';

const QUALITY_OPTIONS = [
  { value: 0, label: 'Blackout', color: 'bg-red-500', desc: 'No recall at all' },
  { value: 1, label: 'Wrong', color: 'bg-red-400', desc: 'Wrong answer, but recognized correct one' },
  { value: 2, label: 'Hard', color: 'bg-orange-400', desc: 'Correct with serious difficulty' },
  { value: 3, label: 'OK', color: 'bg-yellow-400', desc: 'Correct with some hesitation' },
  { value: 4, label: 'Good', color: 'bg-green-400', desc: 'Correct with minor hesitation' },
  { value: 5, label: 'Perfect', color: 'bg-emerald-500', desc: 'Instant perfect recall' },
];
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-10">
        ...
        <p className="text-muted-foreground">Spaced repetition powered reviews to strengthen your memory</p>
      </motion.div>
      ...
      {stats && (
        ...
        >
          ...
          <div className="p-4 rounded-xl border border-border bg-card text-center">
            ...
            <p className="text-2xl font-bold text-foreground">{stats.totalReviewed || 0}</p>
            <p className="text-xs text-muted-foreground">Total Reviewed</p>
          </div>
          <div className="p-4 rounded-xl border border-border bg-card text-center">
            <TrendingUp className="w-5 h-5 mx-auto mb-2 text-primary" />
            <p className="text-2xl font-bold text-foreground">{stats.averageQuality?.toFixed(1) || '—'}</p>
            <p className="text-xs text-muted-foreground">Avg Quality</p>
          </div>
          <div className="p-4 rounded-xl border border-border bg-card text-center">
            <Sparkles className="w-5 h-5 mx-auto mb-2 text-yellow-400" />
            <p className="text-2xl font-bold text-foreground">{stats.masteredCount || 0}</p>
        ...
        >
          ...
          <div className="rounded-2xl border border-border bg-card p-8 min-h-[300px] flex flex-col items-center justify-center text-center">
            ...
            <h2 className="text-xl font-semibold text-foreground mb-6">
              {currentCard.subtopicTitle || currentCard.question || 'What do you remember about this topic?'}
            </h2>
            ...
            {!showAnswer ? (
              ...
              >
                ...
                <p className="text-sm font-medium text-foreground mb-4">How well did you remember?</p>
        ...
        >
          ...
          {sessionResults.length > 0 && (
            <div className="max-w-xl mx-auto mb-8 p-6 rounded-2xl border border-border bg-card">
              <h3 className="text-lg font-semibold text-foreground mb-4 flex items-center justify-center gap-2">
                <CheckCircle className="w-5 h-5 text-green-400" /> Session Complete!
              </h3>
              <div className="flex justify-center gap-6 mb-4">
                <div>
                  <p className="text-2xl font-bold text-foreground">{sessionResults.length}</p>
          ...
          {pendingReviews.length > 0 ? (
            <>
              <div className="mb-6">
                <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-orange-500/10 text-orange-400 text-sm font-medium mb-4">
                  ...
                  {pendingReviews.length} review{pendingReviews.length !== 1 ? 's' : ''} due
                </div>
              </div>
              <motion.button
                whileHover={{ scale: 1.02 }}
            ...
            <div className="p-12 rounded-2xl border border-border bg-card">
              ...
              <p className="text-muted-foreground">No reviews due right now. Great job staying on top of your studies!</p>
            </div>
          )}
        </motion.div>
      )}
