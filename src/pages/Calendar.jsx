...
const Calendar = () => {
  ...
  useEffect(() => {
    ...
    if (searchParams.get('refresh') === 'true') {
      showSuccess('📅 Study plan scheduled successfully! Your sessions are now visible on the calendar.');
    }
  }, []);
    ...
    try {
      ...
      for (const plan of plans) {
        try {
          ...
          const schedule = scheduleResp.data.schedule || scheduleResp.data || null;
          if (schedule && schedule.sessions) {
            // Attach plan id to each session and push
            schedule.sessions.forEach(s => allSessions.push({ ...s, planId: plan.id }));
          }
        } catch (e) {
          console.warn('Failed to fetch schedule for plan', plan.id, e.message || e);
        }
      }
  ...
  const fetchCalendarEntries = async (weeks = 2) => {
    try {
      ...
      const resp = await studyPlanAPI.getCalendar({ weeks, startDate: currentWeekStart.toISOString() });
      ...
      console.log(`✓ SUCCESS: Fetched ${entries.length} calendar entries from backend`);
      ...
      if (err.response?.status === 401) {
        ...
        setError('Authentication failed. Please check the console for token details.');
        ...
        {loading ? (
          ...
          <>
            <div className="week-navigation">
              <button className="nav-btn" onClick={goToPreviousWeek}>⬅️ Previous</button>
              <button className="nav-btn current-week" onClick={goToCurrentWeek}>📅 Current Week</button>
              <button className="nav-btn" onClick={goToNextWeek}>Next ➡️</button>
              <div className="week-range">{formatWeekRange()}</div>
            </div>

            <div className="calendar-controls">
              <label>View weeks:</label>
              <button className={weeksView===1? 'active':''} onClick={() => setWeeksView(1)}>1</button>
              <button className={weeksView===2? 'active':''} onClick={() => setWeeksView(2)}>2</button>
              <button className={weeksView===4? 'active':''} onClick={() => setWeeksView(4)}>4</button>
            </div>
